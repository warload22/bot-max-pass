...
1. Нажмите «Зарегистрироваться». Бот последовательно запрашивает:
   - ФИО.
   - Гражданство (Да/Нет). Если Нет – регистрация прерывается (доступ только для граждан РФ).
   - Год рождения (четыре цифры).
   - Место рождения.
   - Место жительства.
   - Email.
   - Категория (Абитуриент, Родитель, Слушатель). Для абитуриентов дополнительно запрашивается учебное заведение.
   - Интересующее образование (для абитуриентов и родителей).
2. После ввода всех данных бот показывает сводку и предлагает подтвердить или изменить.
3. При подтверждении генерируется QR-код, который отправляется в чат вместе с информацией о необходимости предъявить его на входе вместе с паспортом.
