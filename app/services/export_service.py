...
def export_full_registrations(db, event_id: int) -> bytes:
    ...
    headers = [
        "UUID", "Дата регистрации", "ФИО", "Год рождения", "Место рождения",
        "Место жительства", "Email", "Категория", "Интерес", "Учебное заведение",
        "Гражданин РФ", "Активна", "Последняя отправка QR", "Статус сканирования"
    ]
    ws.append(headers)
    ...
    for reg in registrations:
        ...
        row = [
            ...
            reg.school or '',
            "Да" if reg.is_russian_citizen else "Нет" if reg.is_russian_citizen is not None else "",
...
def export_anonymized_stats(db, event_id: int) -> bytes:
    ...
    headers = [
        ...
        "Гражданин РФ", "Статус сканирования", "Дата регистрации"
    ]
    ws.append(headers)
    ...
    for stat in stats:
        ...
        row = [
            ...
            stat.school or '',
            "Да" if stat.is_russian_citizen else "Нет" if stat.is_russian_citizen is not None else "",
            status_display,
